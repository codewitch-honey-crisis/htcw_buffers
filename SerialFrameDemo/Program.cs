...
internal class Program
{
            ...
            if (sa.Length > 0)
            {
                switch(sa[0])
                {
                    ...
                    case "?":
                        ...
                        Console.Error.WriteLine();
                        Console.Error.WriteLine("MAC\tGets the base MAC address for the ESP32");
                        Console.Error.WriteLine();
    ...
    private static void Session_FrameReceived(object? sender, FrameReceivedEventArgs e)
    {
        ...
        {
            case STMessageCommand.CmdEspIdfVersionResponse:
                {
                    if(STEspIdfVersionResponseMessage.TryRead(e.Data,out var version, out var _)) {
                        Console.WriteLine("ESP-IDF Version Response:");
                        Console.WriteLine($"  {version.Version} ({version.Major}.{version.Minor}.{version.Patch})");
            ...
            case STMessageCommand.CmdGpioGetResponse:
                ...
                break;
            case STMessageCommand.CmdMacAddressResponse:
                {
                    if (STMacAddressResponseMessage.TryRead(e.Data, out var mac, out var _))
                    {
                        Console.WriteLine("MAC Address Response:");
                        Console.WriteLine($"  {string.Join(":", mac.Address.Select(b => b.ToString("X2")))}");
                    }
                }
                break;
            default:
